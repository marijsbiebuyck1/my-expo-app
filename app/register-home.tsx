...
const animalOptions = ['🐱 Kan met katten', '🐶 Kan met honden', '🐰 Kan met knaagdieren'];
const childrenOptions = ['❌ Nee', '🙂 Ja, jonger dan 6 jaar', '👦 Ja, tussen 6 - 14 jaar'];
...
export default function RegisterHome() {
  ...
  const toggleOther = (val: string) => {
    setOtherAnimals(prev => (prev.includes(val) ? prev.filter(p => p !== val) : [...prev, val]));
  };
  ...
  async function onSave() {
    ...
    try {
      ...
      console.debug('PATCH home ->', { userId, hasToken: !!token, homePayload });
      const resp = await fetch(`https://my-express-app-ne4l.onrender.com/users/${userId}/home`, {
        method: 'PATCH',
      ...
      console.debug('PATCH ownerHome response', { status: resp.status, body: respText });
      if (!resp.ok) {
        console.error('Failed to save ownerHome', { status: resp.status, body: respText });
      ...
      Alert.alert('Fout', (err as any)?.message || 'Kon thuissituatie niet opslaan.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <SafeAreaView style={styles.screen} edges={["top", "left", "right", "bottom"]}>
      ...
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        <ThemedText type="title" style={styles.title}>Over je thuissituatie</ThemedText>

        <Text style={styles.sectionLabel}>Heeft toegang tot een tuin?</Text>
        <View style={styles.chipsRow}>
          <TouchableOpacity style={[styles.chip, hasGarden === true && styles.chipSelected]} onPress={() => setHasGarden(true)}>
            <Text style={[styles.chipText, hasGarden === true && styles.chipTextSelected]}>🌳 Ja</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.chip, hasGarden === false && styles.chipSelected]} onPress={() => setHasGarden(false)}>
            <Text style={[styles.chipText, hasGarden === false && styles.chipTextSelected]}>🏙️ Nee</Text>
          </TouchableOpacity>
        </View>

        <Text style={[styles.sectionLabel, { marginTop: 12 }]}>Kan omgaan met andere dieren?</Text>
        <View style={styles.chipsRow}>
          {animalOptions.map(a => (
            <TouchableOpacity key={a} style={[styles.chip, otherAnimals.includes(a) && styles.chipSelected]} onPress={() => toggleOther(a)}>
              <Text style={[styles.chipText, otherAnimals.includes(a) && styles.chipTextSelected]}>{a}</Text>
            </TouchableOpacity>
          ))}
        </View>

        <Text style={[styles.sectionLabel, { marginTop: 12 }]}>Kan omgaan met kinderen</Text>
        <View style={styles.chipsRow}>
...
const styles = StyleSheet.create({
  ...
  sectionLabel: { fontSize: 14, marginBottom: 8, color: '#333', fontFamily: 'Montserrat_700Bold' },
  chipsRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 2 },
  chip: { backgroundColor: '#EFEFD1', paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20, marginRight: 2, marginBottom: 2 },
