...
export default function AdminRegisterHome() {
  ...
  async function onSave() {
    ...
    try {
      ...
      const resp = await api.patch(
        `/asielen/${shelterId}/home`,
        homePayload,
        true
      );

      const respText = await resp.text();
      console.debug("PATCH adminHome response", {
        status: resp.status,
        body: respText,
      });
      if (!resp.ok) {
        console.error("Failed to save adminHome", {
          status: resp.status,
          body: respText,
        });
      ...
      try {
        ...
        console.debug("No JSON body returned from adminHome PATCH");
      }

      router.replace("/admin/home" as any);
    } catch (err) {
      console.error("save adminHome error", err);
      Alert.alert(
        "Fout",
