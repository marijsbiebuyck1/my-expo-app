  ...
  async function onSave() {
    ...
    try {
      ...
      if (!userId) {
        Alert.alert(
          "Fout",
          "Kon je gebruikers-id niet vinden. Probeer in te loggen of registreer opnieuw."
        );
        return;
      }
      ...
      console.debug("PATCH interests response", {
        status: resp.status,
        body: respText,
      });
      if (!resp.ok) {
      ...
      try {
        ...
        console.debug(
          "Saved user after interests (from PATCH body)",
          savedUser
        );
      } catch {
        console.debug(
          "No JSON body returned from interests PATCH (likely 204), will re-fetch user"
        );
      }
      ...
      if (
        ...
        try {
          const getResp = await api.get(`/users/${userId}`);
          if (getResp.ok) {
            const getJson = await getResp.json();
            console.debug("Re-fetched user after interests", getJson);
            savedUser = getJson;
          } else {
            console.warn("Failed to re-fetch user after interests", {
              status: getResp.status,
            });
          }
        } catch (err) {
          console.warn(
            "Network error when re-fetching user after interests",
            err
          );
        }
      }
      ...
      if (savedUser && typeof savedUser === "object") {
        ...
        if (savedUser.id || savedUser._id) {
          await SecureStore.setItemAsync(
            "userId",
            String(savedUser.id ?? savedUser._id)
          );
        }
      }
